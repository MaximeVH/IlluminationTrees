### Implementation of randomly generated decision trees

function fitness(tree, features::Matrix{Float64}, labels)
...
function generate_random_tree(depth::Int, featureIDs::Vector{Int}, features::Matrix{Float64})::AbstractNode
    if depth == 0
        # Create a leaf node with a random majority class
        majority_class = rand(["B", "M"])  # Replace with actual class labels if available
        values = rand(["B", "M"], rand(1:10))  # Random values for the leaf
        return MutableLeaf{Float64, String}(majority_class, values)
    else
        ...
        return MutableNode{Float64, String}(featid, featval, left_subtree, right_subtree)
    end
end

function generate_random_ensemble(num_trees::Int, depth::Int, featureIDs::Vector{Int}, features::Matrix{Float64})
    ...
    return prune_tree.(random_ensemble,0.55)#prune_tree.(ensemble,0.5) #prune the ensemble
end
...
function QD_evolution_RT(forest, features,labels, generations::Int=100, mutation_rate::Float64=0.1, Δ_min::Int=30)
